...
export default function PortfolioXRay() {
    ...
    // Track whether a real PDF upload+analysis is still in flight
    // so the ScanAnimation doesn't prematurely switch to 'result'
    ...
    const onDrop = useCallback((accepted) => {
        if (accepted.length) {
            ...
            setUploadInProgress(true)   // BUG FIX: prevent ScanAnimation race
            setStage('scanning')
            ...
            fetch(`${API_BASE}/mf-xray/upload`, {
                ...
                .catch(err => {
                    ...
                    setUploadInProgress(false)
                    setStage('result') // show mock data on error
                })
        }
    }, [pdfPassword, user?.email])   // BUG FIX: was [] — pdfPassword was always stale empty string
            ...
            <AnimatePresence mode="wait">
                ...
                {stage === 'scanning' && (
                    <motion.div key="scanning" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        {/* BUG FIX: only auto-advance to result if no real upload is in flight */}
                        <ScanAnimation onDone={() => {
                            if (!uploadInProgress) {
